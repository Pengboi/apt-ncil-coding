...
async function fetchAllSets(): Promise<TcgSet[]> {
  ...
  try {
    ...
    const sets: TcgSet[] = await response.json();
 ...
 */
async function fetchCardsFromSet(setId: string): Promise<TcgCard[]> {
  try {
    ...
    const cards: TcgCard[] = setData.cards || [];
 ...
 */
async function insertCard(card: TcgCard, setId: string, setName: string): Promise<boolean> {
  try {
 ...
 */
async function addToTracking(cardId: string, cardName: string): Promise<boolean> {
  try {
    const { error } = await supabase
      .from('tracked_cards')
      .upsert({
