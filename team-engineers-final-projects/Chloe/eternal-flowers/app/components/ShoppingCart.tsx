        ...
        <div className="flex-1 overflow-y-auto p-6">
          {cartItems.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-[var(--charcoal)] text-lg font-display italic mb-2">Your cart is empty</p>
              <p className="text-sm text-[var(--charcoal)]/60 mb-6">Discover our eternal flower collections</p>
              <button 
                onClick={onClose}
                className="text-[var(--charcoal)] text-sm font-medium uppercase tracking-widest hover:text-[var(--burgundy-light)] transition-colors"
              >
                Continue Shopping
              </button>
            </div>
              ...
              {cartItems.map((item, index) => (
                <div key={`${item.id}-${item.selectedColor || 'default'}-${item.ribbonText || 'no-ribbon'}-${index}`} className="flex gap-4 bg-white p-4 rounded-lg border border-[var(--linen)]">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.selectedColor && (
                      <p className="text-xs text-[var(--charcoal)] capitalize mt-0.5">
                        Colour: {item.selectedColor.replace('-', ' ')}
                      </p>
                    )}
                    {item.ribbonText && (
                      <p className="text-xs text-[var(--charcoal)] truncate mt-0.5 font-italic-display">
                        Ribbon: &ldquo;{item.ribbonText}&rdquo;
                      </p>
                    )}
                    {item.glitter && (
                      <p className="text-xs text-[var(--charcoal)] truncate mt-0.5">
                        Glitter Finish (+£5)
                      </p>
                    )}
                  ...
                  <div className="flex flex-col items-end justify-between">
                    <button 
                      ...
                      className="text-[var(--charcoal)] hover:text-[var(--charcoal)] transition-colors"
                    >
                    ...
                    <div className="flex items-center gap-2 bg-[var(--linen)] rounded-full">
                      <button 
                        ...
                        className="w-8 h-8 flex items-center justify-center text-[var(--charcoal)] hover:text-[var(--charcoal)] transition-colors text-sm"
                      >
                        −
                      </button>
        ...
        {cartItems.length > 0 && (
          <div className="border-t border-[var(--linen)] p-6 space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-[var(--charcoal)] uppercase tracking-widest">Subtotal</span>
              <span className="font-display text-3xl font-medium text-[var(--charcoal)]">£{total.toFixed(2)}</span>
            </div>
            <p className="text-xs text-[var(--charcoal)]">Shipping calculated at checkout</p>
            <button 
              onClick={onCheckout}
            ...
            <div className="flex gap-3">
              <button 
                onClick={onClose}
                className="flex-1 py-3 text-[var(--charcoal)] text-xs font-medium uppercase tracking-widest hover:text-[var(--burgundy-light)] transition-colors text-center"
              >
                Continue Shopping
              </button>
              {onClearCart && (
                <button 
                  ...
                  className="px-4 py-3 text-[var(--charcoal)] hover:text-[var(--charcoal)] transition-colors"
                  title="Clear Cart"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
