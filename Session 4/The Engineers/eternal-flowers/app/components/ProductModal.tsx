      ...
      <div className={`relative bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl transition-all duration-500 ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
        ...
        <div className="grid md:grid-cols-2 gap-0">
          ...
          <div className="p-8 md:p-10">
            {/* Category */}
            <span className="inline-block text-[10px] font-medium text-[var(--charcoal)] uppercase tracking-[0.2em] mb-3">
              {product.category}
            </span>
            ...
            <p className="flex items-baseline gap-1 mb-6">
              <span className="text-lg text-[var(--charcoal)]">£</span>
              <span className="text-3xl font-display font-medium text-[var(--charcoal)]">{totalPrice}</span>
              {glitterPrice > 0 && (
                <span className="text-sm text-[var(--charcoal)] ml-2">(includes glitter)</span>
              )}
            </p>

            {/* Description */}
            <p className="text-[var(--charcoal)] mb-6 leading-relaxed text-sm">{product.description}</p>
            ...
            <div className="mb-6">
              ...
              <ul className="space-y-2">
                {product.features.map((feature, idx) => (
                  <li key={idx} className="flex items-center gap-3 text-sm text-[var(--charcoal)]">
            ...
            <div className="mb-6">
              <label className="block text-[10px] font-medium text-[var(--charcoal)] uppercase tracking-[0.2em] mb-3">
                Colour: <span className="text-[var(--charcoal)] font-normal normal-case tracking-normal">{selectedColor?.replace('-', ' ')}</span>
              </label>
            ...
            {product.customizable && (
              <div className="mb-6">
                ...
                <input
                  ...
                  maxLength={30}
                />
                <p className="text-[10px] text-[var(--charcoal)] mt-1">
            ...
            <div className="mb-8">
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-sm text-[var(--charcoal)] font-display italic">+£5</span>
              </label>
              <p className="text-[10px] text-[var(--charcoal)] mt-1 ml-8">
            ...
            <div className="mt-6 pt-6 border-t border-[var(--linen)]">
              <div className="grid grid-cols-2 gap-4 text-xs text-[var(--charcoal)]">
