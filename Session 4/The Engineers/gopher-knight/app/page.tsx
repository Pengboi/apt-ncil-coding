...
function makeCaveMap(): number[][] {
  ...
  for (let y = 0; y < CAVE_ROWS; y++) for (let x = 0; x < CAVE_COLS; x++) {
    ...
    else if (det(x, y, 50) > 0.55) m[y][x] = T.STONE;
    else m[y][x] = T.FLOOR;
  }
  ...
  m[12][12] = T.FLOOR;
  m[12][3] = T.FLOOR; m[12][2] = T.FLOOR; m[11][3] = T.FLOOR; m[13][3] = T.FLOOR; // locked door area
  return m;
}
...
export default function AdventurePage() {
  ...
  const stateRef = useRef({
    ...
    caveBossAlive: true, caveBossMoveTimer: 0,
    caveFade: 1,
  ...
  useEffect(() => {
    ...
    function saveGame() {
      try {
        const data = { px: s.px, py: s.py, level: s.level, xp: s.xp, hp: s.hp, maxHp: s.maxHp, atk: s.atk, def: s.def, mp: s.mp, maxMp: s.maxMp, gold: s.playerGold, inventory: s.inventory, abilities: s.learnedAbilities, sp: s.sp, quests: s.quests, kills: s.totalKills, crafts: s.totalCrafts, timeOfDay: s.timeOfDay, hasCaveKey: s.hasCaveKey };
    ...
    function loadGame() {
      try {
        ...
        if (d.hasCaveKey !== undefined) s.hasCaveKey = d.hasCaveKey;
        return true;
      } catch { return false; }
    }
    ...
    function enterCave() {
      ...
      s.px = 10; s.py = 18;
      if (s.caveBossAlive) { s.caveBossHp = s.caveBossMaxHp; s.caveBossX = 12; s.caveBossY = 12; }
      s.caveKeyDropped = false; s.caveKeyPickedUp = false;
      s.caveBossAggro = false; s.caveBossAttackCooldown = 0;
      const cf = caveFogRef.current; cf.length = 0;
      for (let i = 0; i < 15; i++) cf.push({ x: Math.random() * CAVE_COLS * TILE, y: Math.random() * CAVE_ROWS * TILE, size: 60 + Math.random() * 100, speed: 0.1 + Math.random() * 0.2, opacity: 0.2 + Math.random() * 0.3 });
      playSound('door', 0.6);
    }

    function exitCave() {
      closeAllPanels(); s.caveActive = false; s.caveFade = 1;
      s.px = 14; s.py = 15;
      playSound('door', 1.2);
    }
    ...
    function checkInteraction() {
      ...
      if (s.inNewWorld) {
        ...
        if (Math.abs(px - 3) + Math.abs(py - 12) <= 1.5) {
          if (s.hasCaveKey) { enterNewWorld(); s.interactCooldown = 20; return; }
          else { closeAllPanels(); s.dialogueActive = true; s.dialogueLines = ['A heavy door bars the way...', 'It seems to require a golden key.']; s.dialogueIndex = 0; s.dialogueNpc = ''; playSound('interact'); s.interactCooldown = 20; return; }
        }
      } else {
      if (Math.abs(px - 11) + Math.abs(py - 13) <= 1.5) { enterCave(); s.interactCooldown = 20; return; }
      if (Math.abs(px - 50) + Math.abs(py - 45) <= 1.5 && worldMap[45] && worldMap[45][50] === T.HOUSE_DOOR) { enterShop(); s.interactCooldown = 20; return; }
    ...
    function handleKey(e: KeyboardEvent, down: boolean) {
      ...
      if (k === ' ' && down && !s.battleActive && s.caveActive && !s.dialogueActive && !s.shopActive && !s.signActive && !s.inventoryOpen && !s.craftOpen && !s.menuOpen) {
        e.preventDefault();
        if (s.caveBossAlive && s.playerCaveAttackCooldown <= 0) {
          const dist = Math.abs(s.px - s.caveBossX) + Math.abs(s.py - s.caveBossY);
          if (dist <= 1.5) {
            const dmg = Math.max(1, Math.floor(s.atk * 1.5 + Math.random() * 10));
            s.caveBossHp -= dmg;
            ...
            if (s.caveBossHp <= 0) {
              ...
              s.caveKeyX = s.caveBossX;
              s.caveKeyY = s.caveBossY;
              playSound('levelup');
            } else { playSound('interact', 0.8); }
          }
        }
      }
    ...
    function loop(time: number) {
      ...
      if (s.caveActive) {
        const cm = caveMapRef.current;
        if (s.caveBossAlive && Math.round(newX) === 12 && Math.round(newY) === 12) { s.vx = 0; s.vy = 0; }
        else { if (!isCaveBlocked(Math.round(newX), Math.round(py), cm)) s.px = newX; else s.vx = 0; if (!isCaveBlocked(Math.round(px), Math.round(newY), cm)) s.py = newY; else s.vy = 0; }
      } else if (s.inShop) {
        const sm = shopMapRef.current;
        if (!isShopBlocked(Math.round(newX), Math.round(py), sm)) s.px = newX; else s.vx = 0;
        if (!isShopBlocked(Math.round(px), Math.round(newY), sm)) s.py = newY; else s.vy = 0;
      } else if (s.inNewWorld) {
        const nw = nwMapRef.current;
        if (!isNWBlocked(Math.round(newX), Math.round(py), nw)) s.px = newX; else s.vx = 0;
        if (!isNWBlocked(Math.round(px), Math.round(newY), nw)) s.py = newY; else s.vy = 0;
      } else {
      ...
      if (isMoving && s.footstepCooldown <= 0) {
        ...
        let t = T.GRASS;
        if (ty >= 0 && ty < ROWS && tx >= 0 && tx < COLS) t = worldMap[ty][tx];
        if (s.caveActive) t = T.STONE;
        if (s.inShop) t = T.FLOOR;
      ...
      if (s.caveActive && s.caveKeyDropped && !s.caveKeyPickedUp) {
        const dist = Math.abs(s.px - s.caveKeyX) + Math.abs(s.py - s.caveKeyY);
        if (dist <= 0.8) {
          s.caveKeyPickedUp = true;
          s.hasCaveKey = true;
          s.battleLog.push('You picked up the Golden Key!');
          playSound('levelup');
        }
      }
      ...
      // Cave boss AI
      if (s.caveActive && s.caveBossAlive) {
        const bx = s.caveBossX, by = s.caveBossY, cm = caveMapRef.current;
        const dist = Math.abs(s.px - bx) + Math.abs(s.py - by);
        if (dist < 6) s.caveBossAggro = true;
        if (s.caveBossAggro) {
          s.caveBossAttackCooldown -= dt;
          s.playerCaveAttackCooldown -= dt;
          if (dist <= 1.5 && s.caveBossAttackCooldown <= 0) {
            s.caveBossAttackCooldown = 40;
            const dmg = Math.max(1, Math.floor(30 - s.def * 2 + Math.random() * 15));
            s.hp = Math.max(0, s.hp - dmg);
            s.shakeIntensity = 8;
            if (s.hp <= 0) { s.justDefeated = true; s.hp = 1; }
          }
          if (s.caveBossDir === -1) s.caveBossDir = Math.floor(Math.random() * 4);
          s.caveBossMoveTimer = (s.caveBossMoveTimer || 0) + dt;
          if (s.caveBossMoveTimer > 25) {
            s.caveBossMoveTimer = 0;
            const dirs = [[0, -1], [0, 1], [-1, 0], [1, 0]];
            let nx = bx + dirs[s.caveBossDir][0], ny = by + dirs[s.caveBossDir][1];
            if (nx < 1 || nx >= CAVE_COLS - 1 || ny < 1 || ny >= CAVE_ROWS - 1 || isCaveBlocked(nx, ny, cm) || (nx === 12 && ny === 12)) { s.caveBossDir = Math.floor(Math.random() * 4); nx = bx + dirs[s.caveBossDir][0]; ny = by + dirs[s.caveBossDir][1]; if (!(nx < 1 || nx >= CAVE_COLS - 1 || ny < 1 || ny >= CAVE_ROWS - 1 || isCaveBlocked(nx, ny, cm) || (nx === 12 && ny === 12))) { s.caveBossX = nx; s.caveBossY = ny; } }
            else { s.caveBossX = nx; s.caveBossY = ny; }
          }
        }
        if (s.hp <= 0) { s.justDefeated = true; s.hp = 1; }
      }
      ...
      // Boss respawn timer
      if (s.caveActive && !s.caveBossAlive && s.caveBossRespawnTimer > 0) {
        ...
        if (s.caveBossRespawnTimer <= 0) {
          ...
          s.caveBossMoveTimer = 0;
          s.shakeIntensity = 12;
          if (!s.caveKeyPickedUp) { s.caveKeyDropped = false; }
        }
      }
      ...
      for (const f of caveFogRef.current) { f.x += f.speed * dt; f.y += Math.sin(s.time * 0.01 + f.x * 0.01) * 0.05; if (f.x > camX + W + 200) f.x = camX - 200; if (f.y > camY + H + 200) f.y = camY - 200; }
      ...
      if (s.caveActive) {
        ...
        for (let y = 0; y < CAVE_ROWS; y++) for (let x = 0; x < CAVE_COLS; x++) {
          ...
          const t = cm[y][x];
          if (t === T.STONE || t === T.ROCK) {
            ctx.fillStyle = t === T.ROCK ? '#3a3a3a' : '#1a1a1a';
            ctx.fillRect(sx, sy, TILE, TILE);
            if (t === T.STONE) {
              ctx.fillStyle = '#151515'; ctx.fillRect(sx + 1, sy + 1, TILE - 2, 2);
              ctx.fillStyle = '#111'; ctx.fillRect(sx + 2, sy + 8, 4, 4); ctx.fillRect(sx + 20, sy + 14, 6, 3);
            } else {
              ctx.fillStyle = '#444'; ctx.fillRect(sx + 2, sy + 4, 6, 5); ctx.fillRect(sx + 18, sy + 12, 8, 6);
              ctx.fillStyle = '#333'; ctx.fillRect(sx + 6, sy + 8, 4, 3); ctx.fillRect(sx + 22, sy + 16, 3, 2);
            }
          } else {
            ctx.fillStyle = '#1e1e1e'; ctx.fillRect(sx, sy, TILE, TILE);
            ctx.fillStyle = '#1a1a1a'; ctx.fillRect(sx + 2, sy + 2, TILE - 4, 1); ctx.fillRect(sx + 2, sy + TILE - 3, TILE - 4, 1);
          }
          const isTorch = (x === 4 && y === 5) || (x === 4 && y === 12) || (x === 4 && y === 20) || (x === 10 && y === 4) || (x === 18 && y === 4) || (x === 22 && y === 8) || (x === 22 && y === 14) || (x === 22 && y === 20) || (x === 14 && y === 22) || (x === 6 && y === 22);
          if (isTorch) {
            const flicker = Math.sin(s.time * 0.1 + x + y) * 0.3 + 0.7;
            ctx.fillStyle = '#1a0a00'; ctx.fillRect(sx + 10, sy + 2, 12, 22);
            ctx.shadowColor = `rgba(255,150,50,${flicker * 0.8})`; ctx.shadowBlur = 30 * flicker;
            ctx.fillStyle = `rgba(255,180,60,${flicker * 0.9})`; ctx.beginPath(); ctx.arc(sx + 16, sy + 6, 4 * flicker, 0, Math.PI * 2); ctx.fill();
            ctx.shadowBlur = 0;
            ctx.fillStyle = '#0a0500'; ctx.fillRect(sx + 6, sy + 16, 20, 10);
          }
        }
        // Cave exit indicator
        { const sx = 22 * TILE - camX, sy = 12 * TILE - camY; ctx.fillStyle = 'rgba(0,255,100,0.3)'; ctx.fillRect(sx, sy, TILE, TILE); ctx.fillStyle = 'rgba(0,255,100,0.6)'; ctx.font = '18px monospace'; ctx.textAlign = 'center'; ctx.fillText('▼', sx + 16, sy + 22); }
        // Locked door (left side) - always visible with bright glow
        { const dx = 3 * TILE - camX, dy = 12 * TILE - camY; const pulse = Math.sin(s.time * 0.08) * 0.3 + 0.7;
          const col = s.hasCaveKey ? `hsl(45,100%,${50 + 30 * pulse}%)` : `hsl(0,80%,${30 + 20 * pulse}%)`;
          ctx.shadowColor = s.hasCaveKey ? `rgba(255,215,0,${0.6 * pulse})` : `rgba(200,0,0,${0.5 * pulse})`;
          ctx.shadowBlur = 30 * pulse;
          ctx.fillStyle = col; ctx.fillRect(dx - 2, dy - 2, TILE + 4, TILE + 4);
          ctx.shadowBlur = 0;
          ctx.fillStyle = '#1a0a00'; ctx.fillRect(dx + 2, dy + 2, TILE - 4, TILE - 4);
          ctx.fillStyle = s.hasCaveKey ? '#8a6a2a' : '#5a1a1a'; ctx.fillRect(dx + 3, dy + 3, TILE - 6, TILE - 6);
          ctx.fillStyle = '#2a1a0a'; ctx.fillRect(dx + 5, dy + 4, TILE - 10, 3); ctx.fillRect(dx + 5, dy + 15, TILE - 10, 3);
          ctx.fillStyle = s.hasCaveKey ? '#ffd700' : '#f44'; ctx.font = '22px monospace'; ctx.textAlign = 'center';
          ctx.shadowColor = s.hasCaveKey ? '#ffd700' : '#f44'; ctx.shadowBlur = 15 * pulse;
          ctx.fillText(s.hasCaveKey ? '🔓' : '🔒', dx + 16, dy + 24); ctx.shadowBlur = 0;
          // Label below door
          ctx.fillStyle = `rgba(255,255,200,${0.5 * pulse})`; ctx.font = '10px monospace'; ctx.textAlign = 'center';
          ctx.fillText(s.hasCaveKey ? 'UNLOCKED' : 'LOCKED', dx + 16, dy + 36); }
      } else if (s.inShop) {
        ctx.fillStyle = '#1a1410'; ctx.fillRect(0, 0, W, H);
        const sm = shopMapRef.current;
        for (let y = 0; y < SHOP_ROWS; y++) for (let x = 0; x < SHOP_COLS; x++) {
          const sx = x * TILE - camX, sy = y * TILE - camY;
          if (sx < -TILE || sx > W + TILE || sy < -TILE || sy > H + TILE) continue;
          const t = sm[y][x];
          if (t === SHOP_WALL) {
            ctx.fillStyle = '#2a1a10'; ctx.fillRect(sx, sy, TILE, TILE);
            ctx.fillStyle = '#3a2a1a'; ctx.fillRect(sx + 2, sy + 2, 4, TILE - 4); ctx.fillRect(sx + TILE - 6, sy + 2, 4, TILE - 4);
            ctx.fillStyle = '#1a0a00'; ctx.fillRect(sx + 6, sy + TILE - 6, TILE - 12, 4);
          } else if (t === SHOP_EXIT) {
            ctx.fillStyle = 'rgba(0,255,0,0.15)'; ctx.fillRect(sx, sy, TILE, TILE);
            ctx.fillStyle = 'rgba(0,200,0,0.5)'; ctx.font = '18px monospace'; ctx.textAlign = 'center';
            if (Math.abs(s.px - x) + Math.abs(s.py - y) <= 1.5) ctx.fillText('E', sx + 16, sy + 22);
            else ctx.fillStyle = 'rgba(255,255,255,0.3)';
          } else {
            ctx.fillStyle = '#2a201a'; ctx.fillRect(sx, sy, TILE, TILE);
            ctx.fillStyle = '#322820'; ctx.fillRect(sx, sy, TILE, 1); ctx.fillRect(sx, sy + TILE - 1, TILE, 1);
            ctx.fillStyle = '#221a14'; ctx.fillRect(sx + 3, sy + 3, 8, 2); ctx.fillRect(sx + 17, sy + 8, 6, 2);
          }
        }
        // Merchant NPC
        { const mx = 7 * TILE - camX, my = 4 * TILE - camY; ctx.fillStyle = '#e0b090'; ctx.fillRect(mx + 8, my + 10, 16, 13); ctx.fillStyle = '#c03040'; ctx.fillRect(mx + 8, my + 4, 16, 12); ctx.fillStyle = '#fff'; ctx.fillRect(mx + 12, my + 7, 2, 2); ctx.fillRect(mx + 18, my + 7, 2, 2); ctx.fillStyle = '#111'; ctx.fillRect(mx + 13, my + 8, 1, 1); ctx.fillRect(mx + 19, my + 8, 1, 1); ctx.fillStyle = '#2a2a3a'; ctx.fillRect(mx + 10, my + 23, 5, 5); ctx.fillRect(mx + 17, my + 23, 5, 5); ctx.fillStyle = 'rgba(0,0,0,0.5)'; ctx.fillRect(mx - 8, my - 22, 48, 16); ctx.fillStyle = '#fff'; ctx.font = '14px monospace'; ctx.textAlign = 'center'; ctx.fillText('Merchant', mx + 16, my - 10); }
      } else if (s.inNewWorld) {
        ...
        for (let y = 0; y < NW_ROWS; y++) for (let x = 0; x < NW_COLS; x++) {
          ...
          if (sx < -TILE || sx > W + TILE || sy < -TILE || sy > H + TILE) continue;
          const t = nw[y][x];
          if (t === NW_WALL) { ctx.fillStyle = '#111'; ctx.fillRect(sx, sy, TILE, TILE); ctx.fillStyle = '#0a0a0a'; ctx.fillRect(sx + 2, sy + 2, TILE - 4, TILE - 4); ctx.fillStyle = '#1a1a2a'; ctx.fillRect(sx, sy, TILE, 1); ctx.fillRect(sx, sy + TILE - 1, TILE, 1); ctx.fillRect(sx, sy, 1, TILE); ctx.fillRect(sx + TILE - 1, sy, 1, TILE); }
          else if (t === NW_BUILDING) { ctx.fillStyle = '#1a1a2a'; ctx.fillRect(sx, sy, TILE, TILE); ctx.fillStyle = '#2a2a3a'; ctx.fillRect(sx + 2, sy + 2, TILE - 4, TILE - 4); const wflick = Math.sin(s.time * 0.05 + x * 2 + y * 3) * 0.3 + 0.7; ctx.fillStyle = `rgba(255,200,100,${wflick * 0.15})`; ctx.fillRect(sx + 6, sy + 8, 4, 6); ctx.fillStyle = '#0a0a1a'; ctx.fillRect(sx + 4, sy + 4, TILE - 8, 3); }
          else if (t === NW_EXIT) { ctx.fillStyle = '#0a0a0a'; ctx.fillRect(sx, sy, TILE, TILE); ctx.fillStyle = 'rgba(0,255,200,0.3)'; ctx.fillRect(sx, sy, TILE, TILE); ctx.fillStyle = 'rgba(0,255,200,0.6)'; ctx.font = '18px monospace'; ctx.textAlign = 'center'; ctx.fillText('▲', sx + 16, sy + 22); }
          else { ctx.fillStyle = '#0f0f1f'; ctx.fillRect(sx, sy, TILE, TILE); ctx.fillStyle = '#12122a'; ctx.fillRect(sx + 3, sy + 3, 6, 2); ctx.fillRect(sx + 18, sy + 12, 5, 2); ctx.fillStyle = '#0a0a1a'; ctx.fillRect(sx + 8, sy + 8, 3, 3); }
        }
      } else {
      ...
      // Draw cave entrance marker (pixel cave exit door in main world)
      function drawPixelCaveExitDoor() {
        const cx = 11 * TILE - camX, cy = 13 * TILE - camY;
        ctx.fillStyle = 'rgba(0,0,0,0.4)'; ctx.fillRect(cx + 4, cy + 4, TILE - 8, TILE - 8);
        ctx.fillStyle = '#333'; ctx.fillRect(cx + 2, cy + 2, TILE - 4, TILE - 4);
        ctx.fillStyle = '#1a1a1a'; ctx.fillRect(cx + 4, cy + 4, TILE - 8, TILE - 8);
        ctx.fillStyle = 'rgba(255,255,255,0.15)'; ctx.font = '18px monospace'; ctx.textAlign = 'center'; ctx.fillText('⛰', cx + 16, cy + 22);
      }
      drawPixelCaveExitDoor();

      // Draw magic door (requires golden key)
      { const cx = 15 * TILE - camX, cy = 13 * TILE - camY; const glow = Math.sin(s.time * 0.05) * 0.3 + 0.7; ctx.fillStyle = `rgba(100,200,255,${glow * 0.15})`; ctx.fillRect(cx, cy, TILE, TILE); ctx.fillStyle = `rgba(100,200,255,${glow * 0.3})`; ctx.fillRect(cx + 2, cy + 2, TILE - 4, TILE - 4); ctx.fillStyle = 'rgba(150,220,255,0.4)'; ctx.font = '18px monospace'; ctx.textAlign = 'center'; ctx.fillText('🔮', cx + 16, cy + 22); }
      ...
      if (s.caveActive && s.caveBossAlive) {
        ...
        const bex = bx * TILE - camX, bey = by * TILE - camY;
        if (!(bex < -TILE || bex > W + TILE || bey < -TILE || bey > H + TILE)) {
          const bpulse = Math.sin(s.time * 0.06 + bx + by) * 0.5;
          entities.push({ y: by, draw: () => {
            ctx.fillStyle = `rgba(0,0,0,0.4)`; ctx.beginPath(); ctx.ellipse(bex + TILE / 2, bey + TILE - 2, 12, 5, 0, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = '#8a2a2a'; ctx.fillRect(bex + 6, bey + 8 + bpulse, 20, 16);
            ctx.fillStyle = '#c04040'; ctx.fillRect(bex + 8, bey + 6 + bpulse, 16, 14);
            ctx.fillStyle = '#ff0'; ctx.fillRect(bex + 10, bey + 10 + bpulse, 2, 2); ctx.fillRect(bex + 20, bey + 10 + bpulse, 2, 2);
            ctx.fillStyle = '#a00'; ctx.fillRect(bex + 12, bey + 18 + bpulse, 8, 2);
            ctx.fillStyle = 'rgba(255,0,0,0.2)'; ctx.beginPath(); ctx.arc(bex + 16, bey + 16, 20, 0, Math.PI * 2); ctx.fill();
            // HP bar
            const hpPct = s.caveBossHp / s.caveBossMaxHp;
            ctx.fillStyle = 'rgba(0,0,0,0.6)'; ctx.fillRect(bex - 4, bey - 12, 40, 7);
            ctx.fillStyle = hpPct > 0.5 ? '#0a0' : hpPct > 0.25 ? '#aa0' : '#a00';
            ctx.fillRect(bex - 3, bey - 11, 38 * hpPct, 5);
            ctx.fillStyle = '#fff'; ctx.font = '10px monospace'; ctx.textAlign = 'center'; ctx.fillText('Cave Guardian', bex + 16, bey - 14);
          }});
        }
      }
      ...
      // Cave key entity
      if (s.caveActive && s.caveKeyDropped && !s.caveKeyPickedUp) {
        const kx = s.caveKeyX, ky = s.caveKeyY;
        const kex = kx * TILE - camX, key = ky * TILE - camY;
        if (!(kex < -TILE || kex > W + TILE || key < -TILE || key > H + TILE)) {
          const kglow = Math.sin(s.time * 0.08 + kx + ky) * 0.3 + 0.7;
          entities.push({ y: ky, draw: () => {
            ctx.fillStyle = 'rgba(255,215,0,0.15)'; ctx.shadowColor = '#ffd700'; ctx.shadowBlur = 20 * kglow;
            ctx.beginPath(); ctx.arc(kex + 16, key + 16, 10, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0;
            ctx.fillStyle = '#ffd700'; ctx.font = '20px monospace'; ctx.textAlign = 'center'; ctx.fillText('🔑', kex + 16, key + 22);
            ctx.shadowColor = '#ffd700'; ctx.shadowBlur = 15 * kglow; ctx.fillStyle = 'rgba(255,215,0,0.5)'; ctx.beginPath(); ctx.arc(kex + 16, key + 16, 8 * kglow, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0;
          }});
        }
      }
      ...
      if (s.caveActive) for (const f of caveFogRef.current) { const fx = f.x - camX, fy = f.y - camY; ctx.fillStyle = `rgba(0,0,0,${f.opacity * 0.5})`; ctx.beginPath(); ctx.arc(fx, fy, f.size, 0, Math.PI * 2); ctx.fill(); }
      ...
      if (!s.dialogueActive && !s.shopActive && !s.signActive && !s.cutsceneActive && !s.battleActive && !s.inventoryOpen && !s.craftOpen && !s.menuOpen) {
        ...
        if (s.caveActive) {
          if (pxi >= 22 && pyi === 12) { showE = true; label = 'Exit'; }
          if (Math.abs(pxi - 3) + Math.abs(pyi - 12) <= 1.5) { showE = true; label = s.hasCaveKey ? 'Unlock' : 'Locked'; }
        } else if (s.inShop) {
          if (pxi >= 8 && pyi === 10) { showE = true; label = 'Exit'; }
          else if (Math.abs(pxi - 7) + Math.abs(pyi - 4) <= 1.5) { showE = true; label = 'Shop'; }
        } else if (s.inNewWorld) {
          if (Math.abs(pxi - (NW_COLS - 3)) + Math.abs(pyi - 3) <= 1.5) { showE = true; label = 'Exit'; }
        } else {
          if (Math.abs(pxi - 11) + Math.abs(pyi - 13) <= 1.5) { showE = true; label = 'Enter'; }
          if (Math.abs(pxi - 15) + Math.abs(pyi - 13) <= 1.5) { showE = true; label = s.hasCaveKey ? 'Enter' : 'Locked'; }
          if (Math.abs(pxi - 50) + Math.abs(pyi - 45) <= 1.5 && worldMap[45] && worldMap[45][50] === T.HOUSE_DOOR) { showE = true; label = 'Shop'; }
        }
      ...
      // --- Cave HUD ---
      if (s.caveActive) {
        ctx.fillStyle = 'rgba(0,0,0,0.5)'; ctx.fillRect(8, 8, 200, s.caveBossAlive ? 56 : 48);
        if (s.caveBossAlive) {
          const hpPct = Math.max(0, s.caveBossHp / s.caveBossMaxHp);
          ctx.fillStyle = '#f88'; ctx.font = '13px monospace'; ctx.textAlign = 'left'; ctx.fillText('Cave Guardian', 16, 26);
          ctx.fillStyle = '#400'; ctx.fillRect(16, 32, 180, 12);
          ctx.fillStyle = hpPct > 0.5 ? '#0a0' : hpPct > 0.25 ? '#aa0' : '#a00';
          ctx.fillRect(16, 32, 180 * hpPct, 12);
          ctx.fillStyle = '#fff'; ctx.font = '10px monospace'; ctx.fillText(`${Math.ceil(s.caveBossHp)}/${s.caveBossMaxHp}`, 16, 52);
        } else if (!s.caveBossAlive && s.caveBossRespawnTimer > 0) {
          const secs = Math.ceil(s.caveBossRespawnTimer / 60);
          ctx.fillStyle = '#f88'; ctx.font = '13px monospace'; ctx.textAlign = 'left';
          ctx.fillText('Boss respawns in:', 16, 26);
          ctx.fillStyle = '#ff0'; ctx.font = '18px monospace';
          ctx.fillText(`${secs}s`, 16, 48);
        }
      }
